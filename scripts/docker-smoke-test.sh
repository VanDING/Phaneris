#!/usr/bin/env bash
# =============================================================================
# Docker smoke test for Phaneris Server
#
...
docker run -d \
  --name "$CONTAINER_NAME" \
  -p "$PORT:9100" \
  -e "PHANERIS_SERVER_TOKEN=$TOKEN" \
  -e "PHANERIS_RPC_HOST=0.0.0.0" \
  -e "PHANERIS_RPC_PORT=9100" \
  "$IMAGE"
...
while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
  ...
  if docker logs "$CONTAINER_NAME" 2>&1 | grep -q "PHANERIS_SERVER_URL="; then
    READY=true
    break
  fi
