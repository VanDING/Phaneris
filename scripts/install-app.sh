...
# Base URL of a release feed THIS PROJECT controls, using the electron-updater
# generic layout (<base>/<channel>/latest.yml). There is no Phaneris-hosted
# endpoint yet (fork plan phase D), and falling back to the upstream service
# would install a different product under this installer's name — so the script
# refuses to run without it rather than guessing.
VERSIONS_URL="${PHANERIS_RELEASE_BASE_URL:-}"
DOWNLOAD_DIR="$HOME/.phaneris/downloads"

if [ -z "$VERSIONS_URL" ]; then
    printf "%b\n" "x No Phaneris release channel is configured."
    printf "%b\n" "  Set PHANERIS_RELEASE_BASE_URL to the base URL of a release feed this"
    printf "%b\n" "  project controls (electron-updater generic layout: <base>/latest/latest.yml),"
    printf "%b\n" "  then re-run. This installer will not download from the upstream service."
    exit 1
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Phaneris-x64.AppImage"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="io.github.vanding.phaneris"
    if pgrep -x "Phaneris" >/dev/null 2>&1; then
        info "Quitting Phaneris..."
        ...
        if pgrep -x "Phaneris" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Phaneris" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Phaneris has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Phaneris'${NC}"
    echo ""
    ...
    # New paths
    APP_DIR="$HOME/.phaneris/app"
    WRAPPER_PATH="$INSTALL_DIR/phaneris"
    APPIMAGE_INSTALL_PATH="$APP_DIR/Phaneris-x64.AppImage"

    # Kill the app if it's running
    if pgrep -f "Phaneris.*AppImage" >/dev/null 2>&1; then
        info "Stopping Phaneris..."
        pkill -f "Phaneris.*AppImage" 2>/dev/null || true
        sleep 2
    fi
...
#!/bin/bash
# Phaneris launcher - handles Linux-specific AppImage issues

APPIMAGE_PATH="$HOME/.phaneris/app/Phaneris-x64.AppImage"
ELECTRON_CACHE="$HOME/.config/@phaneris"
ELECTRON_CACHE_ALT="$HOME/.cache/@phaneris"

# Verify AppImage exists
if [ ! -f "$APPIMAGE_PATH" ]; then
    echo "Error: Phaneris not found at $APPIMAGE_PATH"
    echo "Reinstall: run this project's own install-app.sh from its repository"
    exit 1
fi
...
# AppImage creates a new /tmp/.mount_Phaneris-XXXX each launch, so any cached path is stale
for cache_dir in "$ELECTRON_CACHE" "$ELECTRON_CACHE_ALT"; do
    if [ -d "$cache_dir" ] && grep -rq '/tmp/\.mount_Phaneris' "$cache_dir" 2>/dev/null; then
        rm -rf "$cache_dir"
    fi
done
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Phaneris-x64.AppImage"
