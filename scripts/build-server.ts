#!/usr/bin/env bun
/**
 * Build script for standalone Phaneris server.
 *
...
function showHelp(): void {
  console.log(`
Standalone server build script for Phaneris
 ...
 * not node: builtins, not workspace @phaneris/* packages).
 */
    ...
    for (const entry of readdirSync(d, { withFileTypes: true })) {
      ...
      if (entry.isDirectory() && entry.name !== 'node_modules' && entry.name !== 'tests' && entry.name !== '__tests__') {
        ...
        while ((match = importRe.exec(content)) !== null) {
          ...
          if (spec.startsWith('.') || spec.startsWith('node:') || spec.startsWith('@phaneris/')) continue;
...
function createRootConfig(config: ServerBuildConfig): void {
  ...
  const rootTsconfig = {
    compilerOptions: {
      ...
      paths: {
        '@phaneris/server-core/*': ['./packages/server-core/src/*'],
        '@phaneris/shared/*': ['./packages/shared/src/*'],
        '@phaneris/core/*': ['./packages/core/src/*'],
        '@phaneris/session-tools-core/*': ['./packages/session-tools-core/src/*'],
      },
    },
  };
  writeFileSync(join(outputDir, 'tsconfig.json'), JSON.stringify(rootTsconfig, null, 2) + '\n');

  // Create workspace symlinks in node_modules/@phaneris/
  // Bun needs these to resolve workspace package imports at runtime
  const scopeDir = join(outputDir, 'node_modules', '@phaneris');
  ...
  if (existsSync(packagesDir)) {
    for (const pkg of readdirSync(packagesDir)) {
      ...
      try {
        ...
        if (name.startsWith('@phaneris/')) {
          const shortName = name.replace('@phaneris/', '');
...
async function main(): Promise<void> {
  ...
  console.log(`=== Building Phaneris Server ${version} for ${platform}-${arch} ===`);
