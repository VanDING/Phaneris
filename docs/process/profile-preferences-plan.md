...
2. **我怎样使用 Phaneris**：会话、Token、活跃、能力与工作方式洞察。
3. **Phaneris 应如何配合我**：原 Preferences 的结构化字段、长期偏好和自由备注。
...
- 给 Phaneris 的说明：沿用自由备注，并保留 AI 辅助编辑与直接编辑文件能力。
...
- **跨主题一致**：只使用 Phaneris 六色系统及语义 Token，不引入 Profile 专属品牌色。
