...
保留自研，不引入主题框架；把现有"6 色 + 双锚点派生"升级为 **语义 token 分层 + 一个高阶 `--depth` 深度维度**，覆盖 **L1 色彩 / L2 形状 / L3 材质 / L4 排版 / L5 图标 / L6 密度** 六层自由度，让"换 token 换风格"成立。产品只内置含 light/dark 的 `default`；其他主题全部从用户目录 `~/.phaneris/themes/` 读取。
...
- `~/.phaneris/themes/*.json` 是所有非默认主题的唯一来源。该目录不会被应用播种、覆盖、重置或清理；旧版本已经留下的主题文件继续作为普通用户主题使用。
...
主题引擎已合入 `main`（`docs/README.md` 将其标为 Implemented）；历史上的 `Theme` 分支已不存在，本文描述的行为即当前实现。
