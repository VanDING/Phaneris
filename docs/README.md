...
| [Agentic interception design](agentic-interception-design.md) | Design; positioning decided (option B — governance and intervention), implementation not started | Why agentic capability is interception rather than automation, the Pi extension-hook primitives it rests on (`block` / `cancel` / `transform`), the decision-in-subprocess vs authority-in-main-process boundary, and the `tool_call` blocking first slice. Also records why agent-event automations execute nothing today. |
...
| [Product development directions](product-development-directions-2026-09-19.md) | Discussion draft (2026-09-19); direction selection and priorities pending | Benchmark-informed capability directions built on the current code: reliability guards and budgets, command-level policy rules, context and tool budgets, orchestration, completion metrics and evaluation, lifecycle hooks, and an external control surface — each with a first slice and an explicit not-doing list. |
