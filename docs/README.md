...
| [Plugin bundles design](plugin-bundles-design.md) | Proposed | Aggregate skills, sources, and a prompt fragment into an Agent Plugins 1.0.0 package; `/name` invocation, session-resident, no lifecycle. Paired with [its decision register](plugin-bundles-decisions.md). |
| [Plugin bundles decision register](plugin-bundles-decisions.md) | Proposed, blocking | Every open decision that must be closed before implementation; no item may remain open at implementation start. |
