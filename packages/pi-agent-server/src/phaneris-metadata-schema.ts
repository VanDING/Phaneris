const PHANERIS_DISPLAY_NAME_KEY = '_displayName';
const PHANERIS_INTENT_KEY = '_intent';
...
export function allowPhanerisMetadataProperties<T>(schema: T): T {
  ...
  if (!(PHANERIS_DISPLAY_NAME_KEY in nextProperties)) {
    nextProperties[PHANERIS_DISPLAY_NAME_KEY] = PHANERIS_DISPLAY_NAME_SCHEMA;
  }
  if (!(PHANERIS_INTENT_KEY in nextProperties)) {
    nextProperties[PHANERIS_INTENT_KEY] = PHANERIS_INTENT_SCHEMA;
  }
...
export function stripPhanerisMetadata<T>(input: T): T {
  ...
  delete cleanInput[PHANERIS_DISPLAY_NAME_KEY];
  delete cleanInput[PHANERIS_INTENT_KEY];
