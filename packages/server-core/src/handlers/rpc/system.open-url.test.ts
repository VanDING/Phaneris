...
import type { RpcServer, HandlerFn, RequestContext } from '@phaneris/server-core/transport'
...
describe('registerSystemCoreHandlers OPEN_URL', () => {
  it('routes craftagents action links internally via deeplink:navigate', async () => {
    ...
    await openUrl(ctx, 'phaneris://action/new-session?input=sg&send=true')
  ...
  it('routes workspace deep links to workspace target when URL workspace differs', async () => {
    ...
    await openUrl(ctx, 'phaneris://workspace/ws-2/action/new-session?input=hello')
  ...
  it('falls back to client openExternal for craftagents window-mode links', async () => {
    ...
    await openUrl(ctx, 'phaneris://action/new-session?window=focused')
    ...
    expect(invokeClientCalls[0]).toEqual({
      ...
      args: ['phaneris://action/new-session?window=focused'],
    })
  })
