...
import { RPC_CHANNELS, type SkillFile } from '@phaneris/shared/protocol'
...
export function registerSkillsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.skills.GET_FILES, async (_ctx, workspaceId: string, skillSlug: string) => {
    ...
    const { getWorkspaceSkillsPath } = await import('@phaneris/shared/workspaces')
  ...
  server.handle(RPC_CHANNELS.skills.DELETE, async (_ctx, workspaceId: string, skillSlug: string) => {
    ...
    const { deleteSkill } = await import('@phaneris/shared/skills')
