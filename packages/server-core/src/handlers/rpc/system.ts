...
import { getWorkspaceByNameOrId, getGitBashPath, setGitBashPath, clearGitBashPath } from '@phaneris/shared/config'
...
function parseInternalDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== DEEPLINK_PROTOCOL) return null
    ...
    try {
      ...
      if (classification.kind === 'internal-deeplink') {
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing phaneris:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for phaneris:// URL')
