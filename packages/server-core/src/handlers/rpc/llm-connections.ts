import { RPC_CHANNELS, type LlmConnectionSetup } from '@phaneris/shared/protocol'
import { getLlmConnections, getLlmConnection, addLlmConnection, updateLlmConnection, deleteLlmConnection, getDefaultLlmConnection, setDefaultLlmConnection, touchLlmConnection, isCompatProvider, getDefaultModelsForConnection, getDefaultModelForConnection, type LlmConnection, type LlmConnectionWithStatus, toBedrockNativeId, deriveBedrockRegionPrefix } from '@phaneris/shared/config'
import { getCredentialManager } from '@phaneris/shared/credentials'
import { setSetupDeferred } from '@phaneris/shared/config/storage'
import {
...
import { CLIENT_OPEN_EXTERNAL } from '@phaneris/server-core/transport'
...
export function registerLlmConnectionsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.pi.GET_API_KEY_PROVIDERS, async () => {
    const { getPiApiKeyProviders } = await import('@phaneris/shared/config')
    return getPiApiKeyProviders()
  })

  server.handle(RPC_CHANNELS.pi.GET_PROVIDER_BASE_URL, async (_ctx, provider: string) => {
    const { getPiProviderBaseUrl } = await import('@phaneris/shared/config')
  ...
  server.handle(RPC_CHANNELS.llmConnections.TEST, async (_ctx, slug: string): Promise<{ success: boolean; error?: string }> => {
    try {
      ...
      const { parseValidationError } = await import('@phaneris/shared/config')
  ...
  server.handle(RPC_CHANNELS.chatgpt.START_OAUTH, async (ctx, connectionSlug: string): Promise<{
    ...
    const { prepareChatGptOAuth } = await import('@phaneris/shared/auth')
  ...
  server.handle(RPC_CHANNELS.chatgpt.COMPLETE_OAUTH, async (ctx, args: {
    ...
    try {
      const { exchangeChatGptTokens } = await import('@phaneris/shared/auth')
      const credentialManager = getCredentialManager()
