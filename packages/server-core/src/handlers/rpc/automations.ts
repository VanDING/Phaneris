...
import { appendAutomationHistoryEntry } from '@phaneris/shared/automations/history-store'
import { AUTOMATION_HISTORY_MAX_RUNS_PER_MATCHER } from '@phaneris/shared/automations/constants'
...
async function withAutomationMatcher(workspaceId: string, eventName: string, matcherIndex: number, mutate: (matchers: Record<string, unknown>[], index: number, config: AutomationsConfigJson, genId: () => string) => void) {
  ...
  await withConfigMutex(workspace.rootPath, async () => {
    const { resolveAutomationsConfigPath, generateShortId } = await import('@phaneris/shared/automations/resolve-config-path')
...
export function registerAutomationsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.automations.GET, async (_ctx, workspaceId: string) => {
    ...
    try {
      const { resolveAutomationsConfigPath } = await import('@phaneris/shared/automations/resolve-config-path')
      const configPath = resolveAutomationsConfigPath(workspace.rootPath)
  ...
  server.handle(RPC_CHANNELS.automations.TEST, async (_ctx, payload: import('@phaneris/shared/protocol').TestAutomationPayload) => {
    const workspace = getWorkspaceByNameOrId(payload.workspaceId)
    if (!workspace) throw new Error('Workspace not found')

    const results: import('@phaneris/shared/protocol').TestAutomationActionResult[] = []
    const { parsePromptReferences } = await import('@phaneris/shared/automations')
    const { executeWebhookRequest, createWebhookHistoryEntry, createPromptHistoryEntry } = await import('@phaneris/shared/automations/webhook-utils')
    ...
    for (const action of payload.actions) {
      ...
      if (action.type === 'webhook') {
        ...
        const result = await executeWebhookRequest(action as import('@phaneris/shared/automations').WebhookAction)
      ...
      if (action.type === 'script') {
        ...
        const { executeScriptAction, createScriptHistoryEntry, buildScriptEnv } = await import('@phaneris/shared/automations')
    ...
    return { actions: results } satisfies import('@phaneris/shared/protocol').TestAutomationResult
  })
  ...
  server.handle(RPC_CHANNELS.automations.REPLAY, async (_ctx, workspaceId: string, automationId: string, eventName: string) => {
    ...
    const { executeWebhookRequest, createWebhookHistoryEntry } = await import('@phaneris/shared/automations/webhook-utils')
    const results = await Promise.all(
      webhookActions.map(a => executeWebhookRequest(a as unknown as import('@phaneris/shared/automations').WebhookAction))
    )
