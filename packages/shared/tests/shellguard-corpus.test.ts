...
const TEST_MODE_CONFIG = {
  ...
  readOnlyBashPatterns: [
    ...
    // phaneris CLI read-only
    { regex: /^phaneris\s+label\s+(list|get)\b/, source: '^phaneris\\s+label\\s+(list|get)\\b', comment: 'phaneris label read-only operations' },
    { regex: /^phaneris\s+source\s+(list|get|validate|test)\b/, source: '^phaneris\\s+source\\s+(list|get|validate|test)\\b', comment: 'phaneris source read-only operations' },
    { regex: /^phaneris\s+skill\s+(list|get|validate|where)\b/, source: '^phaneris\\s+skill\\s+(list|get|validate|where)\\b', comment: 'phaneris skill read-only operations' },
    { regex: /^phaneris\s+automation\s+(list|get|validate|history|last-executed|test|lint)\b/, source: '^phaneris\\s+automation\\s+(list|get|validate|history|last-executed|test|lint)\\b', comment: 'phaneris automation read-only operations' },
    { regex: /^phaneris\s*$/, source: '^phaneris\\s*$', comment: 'phaneris bare invocation' },
    { regex: /^phaneris\s+(label|source|skill|automation)\s+--help\b/, source: '^phaneris\\s+(label|source|skill|automation)\\s+--help\\b', comment: 'phaneris entity help flags' },
    { regex: /^phaneris\s+--(help|version|discover)\b/, source: '^phaneris\\s+--(help|version|discover)\\b', comment: 'phaneris global flags' },
...
// ============================================================
// Group 27: phaneris CLI allowlist
// ============================================================

describe('ShellGuard corpus: phaneris CLI allowlist', () => {
  const shouldAllow = [
    'phaneris',
    'phaneris --help',
    'phaneris --version',
    ...
    'phaneris skill --help',
    'phaneris skill list',
    'phaneris skill get commit-helper',
    'phaneris skill where commit-helper',
    'phaneris skill validate commit-helper',
    'phaneris automation list',
    'phaneris automation get abc123',
    'phaneris automation validate',
    'phaneris automation history abc123 --limit 5',
    'phaneris automation last-executed abc123',
    'phaneris automation test abc123 --match "x"',
    'phaneris automation lint',
  ];

  const shouldBlock = [
    'phaneris label create --name Bug',
    'phaneris label update bug --name "Bug Report"',
    'phaneris label delete bug',
    'phaneris label move bug --parent root',
    'phaneris label reorder --parent root a b c',
    'phaneris source create --name Linear --provider linear --type mcp',
    'phaneris source update linear --json "{\"enabled\":false}"',
    'phaneris source delete linear',
    'phaneris skill create --name "Review" --description "x"',
    'phaneris skill update review --json "{\"description\":\"y\"}"',
    'phaneris skill delete review',
    'phaneris automation create --event UserPromptSubmit --prompt "x"',
    'phaneris automation update abc123 --json "{\"enabled\":false}"',
    'phaneris automation delete abc123',
    'phaneris automation enable abc123',
    'phaneris automation disable abc123',
