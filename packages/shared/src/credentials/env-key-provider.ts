...
/**
 * Headless/server key provider. `PHANERIS_CREDENTIAL_KEY` carries a high-entropy
...
export function installCredentialKeyProviderFromEnv(
  ...
  const raw = env.PHANERIS_CREDENTIAL_KEY;
  if (!raw?.trim()) return null;

  const key = decodeKey(raw);
  if (!key) {
    throw new Error(
      'PHANERIS_CREDENTIAL_KEY must be 16-64 bytes encoded as hex or base64',
    );
  }

  const provider: CredentialKeyProvider = {
    id: 'env:PHANERIS_CREDENTIAL_KEY',
    getKey: () => key,
  };
