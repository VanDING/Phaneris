 ...
 *
 * Supports multi-instance development via PHANERIS_CONFIG_DIR environment variable.
 * When running from a numbered folder (e.g., craft-tui-agent-1), the detect-instance.sh
 * script sets PHANERIS_CONFIG_DIR to ~/.craft-agent-1, allowing multiple instances to run
...
export const CONFIG_DIR = process.env.PHANERIS_CONFIG_DIR || join(homedir(), '.craft-agent');
