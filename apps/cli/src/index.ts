...
export function parseArgs(argv: string[]): CliArgs {
  ...
  // Env var fallbacks
  if (!url) url = process.env.PHANERIS_SERVER_URL ?? ''
  if (!token) token = process.env.PHANERIS_SERVER_TOKEN ?? ''
  if (!tlsCa) tlsCa = process.env.PHANERIS_TLS_CA
...
Connection:
  ...
  --token <secret>       Auth token (default: $PHANERIS_SERVER_TOKEN)
...
export async function main(argv: string[] = process.argv): Promise<void> {
  ...
  if (!args.url) {
    err('No server URL. Use --url <ws://...> or set $PHANERIS_SERVER_URL')
    process.exit(1)
  }
