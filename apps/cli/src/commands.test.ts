...
describe('parseArgs', () => {
  ...
  it('falls back to env vars for url and token', () => {
    const prevUrl = process.env.PHANERIS_SERVER_URL
    const prevToken = process.env.PHANERIS_SERVER_TOKEN
    const prevCa = process.env.PHANERIS_TLS_CA

    process.env.PHANERIS_SERVER_URL = 'ws://env-server:8080'
    process.env.PHANERIS_SERVER_TOKEN = 'env-token'
    process.env.PHANERIS_TLS_CA = '/env/ca.pem'
    ...
    try {
      ...
      if (prevUrl === undefined) delete process.env.PHANERIS_SERVER_URL
      else process.env.PHANERIS_SERVER_URL = prevUrl
      if (prevToken === undefined) delete process.env.PHANERIS_SERVER_TOKEN
      else process.env.PHANERIS_SERVER_TOKEN = prevToken
      if (prevCa === undefined) delete process.env.PHANERIS_TLS_CA
      else process.env.PHANERIS_TLS_CA = prevCa
    }
  })
