 ...
 *
 * Warns against using localStorage in Phaneris codebase.
 * All persistent user settings should be stored in file-based configs
 * (preferences.json, workspace configs) for consistency with Phaneris
...
module.exports = {
  meta: {
    ...
    messages: {
      noLocalStorage:
        "Avoid localStorage in Phaneris. Store settings in ~/.craft-agent/preferences.json using window.electronAPI.readPreferences/writePreferences. See packages/shared/src/config/preferences.ts for the preferences API.",
    },
    schema: [],
  },
