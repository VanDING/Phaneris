 ...
 *
 * Thin-client mode (PHANERIS_SERVER_URL):
...
import { createCallbackServer } from '@phaneris/shared/auth/callback-server'
import { CHATGPT_OAUTH_CONFIG } from '@phaneris/shared/auth/chatgpt-oauth-config'
import {
...
import type { ConfirmDialogSpec, FileDialogSpec, BrowserCapabilityRequest } from '@phaneris/server-core/transport'
...
if (isClientOnly) {
  ...
  const wsUrl = process.env.PHANERIS_SERVER_URL!
  const wsToken = process.env.PHANERIS_SERVER_TOKEN ?? ''
  ...
  if (parsed.protocol === 'ws:' && !isLocalhost) {
    throw new Error(
      ...
      `Set PHANERIS_RPC_TLS_CERT/KEY on the server to enable TLS.`
    )
  }

  // Workspace ID is optional — if missing, renderer shows a workspace picker
  const workspaceId = process.env.PHANERIS_WORKSPACE_ID || ipcRenderer.sendSync('__get-workspace-id') || undefined
...
;(api as ElectronAPI).getSystemWarnings = async () => ({
  vcredistMissing: process.env.PHANERIS_VCREDIST_MISSING === '1',
  downloadUrl: process.env.PHANERIS_VCREDIST_URL,
})
