...
export class WindowManager {
  ...
  private eventSink: ((channel: string, target: import('@phaneris/shared/protocol').PushTarget, ...args: any[]) => void) | null = null
   ...
   */
  setRpcEventSink(
    sink: (channel: string, target: import('@phaneris/shared/protocol').PushTarget, ...args: any[]) => void,
  ...
  getRpcEventSink(): ((channel: string, target: import('@phaneris/shared/protocol').PushTarget, ...args: any[]) => void) | null {
   ...
   *   1 window  → app name ("Phaneris") on the lone window
  ...
  createWindow(options: CreateWindowOptions): BrowserWindow {
    ...
    // The renderer's index.html ships with `<title>Phaneris</title>`, so
