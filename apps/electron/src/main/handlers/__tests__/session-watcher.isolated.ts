...
import type { RpcServer, RequestContext } from '@phaneris/server-core/transport'
...
describe('session file watcher isolation', () => {
  ...
  it('creates independent watchers per client and cleans up on disconnect', async () => {
    ...
    const { registerSessionsHandlers, cleanupSessionFileWatchForClient } = await import('@phaneris/server-core/handlers/rpc')
