...
import { getGitBashPath, setGitBashPath, clearGitBashPath } from '@phaneris/shared/config'
import { classifyExternalUrl, formatBlockedUrlError } from '@phaneris/shared/utils/url-safety'
import { isUsableGitBashPath, validateGitBashPath } from '@phaneris/server-core/services'
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.releaseNotes.GET, async () => {
    const { getCombinedReleaseNotes } = require('@phaneris/shared/release-notes') as typeof import('@phaneris/shared/release-notes')
    return getCombinedReleaseNotes()
  })

  server.handle(RPC_CHANNELS.releaseNotes.GET_LATEST_VERSION, async () => {
    const { getLatestReleaseVersion } = require('@phaneris/shared/release-notes') as typeof import('@phaneris/shared/release-notes')
  ...
  // Shell operations - open URL in external browser (or handle phaneris:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      // Handle phaneris:// URLs internally via deep link handler (GUI only)
...
export function registerSystemGuiHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.update.DISMISS, async (_ctx, version: string) => {
    const { setDismissedUpdateVersion } = await import('@phaneris/shared/config')
    setDismissedUpdateVersion(version)
  })

  server.handle(RPC_CHANNELS.update.GET_DISMISSED, async () => {
    const { getDismissedUpdateVersion } = await import('@phaneris/shared/config')
  ...
  server.handle(RPC_CHANNELS.notification.GET_ENABLED, async () => {
    const { getNotificationsEnabled } = await import('@phaneris/shared/config/storage')
