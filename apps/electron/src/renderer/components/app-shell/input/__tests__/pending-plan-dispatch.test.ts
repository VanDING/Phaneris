/**
 * Persisted plan dispatch must send exactly one approval per pending record, from
 * whichever window or listener gets there first, and only when the host's atomic
 * claim says this caller owns it (see `pending-plan-dispatch.ts`).
 */
import { describe, expect, it } from 'bun:test'

import { createPendingPlanDispatcher, type PendingPlanExecution } from '../pending-plan-dispatch'

const ready: PendingPlanExecution = {
  planPath: '/plan.md',
  draftInputSnapshot: 'Keep this',
...
describe('persisted plan dispatch', () => {
  it('waits for idle, skips an unready or cleared record, and sends once it is ready', async () => {
    let busy = true
    ...
    expect(claims).toBe(0) // busy: nothing is even read
  ...
  it('requires claim === true, so duplicate listeners and a reload send once', async () => {
    let claimed = false
    ...
    const dependencies = {
      ...
      submit: (pending: PendingPlanExecution) => { sent.push(pending) },
      clear: async () => {},
    }
  ...
  it('rechecks busy after loading, and disposal cancels an unclaimed read', async () => {
