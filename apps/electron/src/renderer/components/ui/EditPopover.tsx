...
import { usePlatform } from '@phaneris/ui'
...
const EDIT_CONFIGS: Record<EditContextKey, (location: string) => EditConfig> = {
  ...
  'default-permissions': (location) => ({
    context: {
      ...
      context:
        'The user is editing app-level default permissions (~/.phaneris/permissions/default.json). ' +
  ...
  'preferences-notes': (location) => ({
    context: {
      ...
      context:
        'The user is editing the notes field in their preferences (~/.phaneris/preferences.json). ' +
  ...
  'add-source': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.phaneris/docs/sources.md. ' +
  ...
  'add-skill': (location) => ({
    context: {
      ...
      context:
        ...
        'Follow the patterns in ~/.phaneris/docs/skills.md. ' +
  ...
  'edit-labels': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.phaneris/docs/labels.md for full format reference. ' +
        'Confirm clearly when done.',
    },
  ...
  'edit-tool-icons': (location) => ({
    context: {
      ...
      context:
        ...
        'The file is tool-icons.json in ~/.phaneris/tool-icons/. Icon image files live in the same directory. ' +
        ...
        'Read ~/.phaneris/docs/tool-icons.md for full format reference. ' +
  ...
  'automation-config': (location) => ({
    context: {
      ...
      context:
        ...
        'Read ~/.phaneris/docs/automations.md for full format reference. ' +
...
export function EditPopover({
  ...
  const { onCreateSession, onSendMessage, onRespondToPermission, onRespondToCredential, onRespondToAskUser } = useAppShellContext()
  ...
  // Pending permission/credential/question requests for inline session (same flow as main chat)
  ...
  const handleLegacySendMessage = useCallback((message: string) => {
    ...
    const url = `phaneris://action/new-session?input=${encodedInput}&send=true&mode=${permissionMode}&badges=${encodedBadges}${workdirParam}${modelParam}${systemPromptParam}`
