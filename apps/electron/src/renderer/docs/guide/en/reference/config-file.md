...
| `promptCacheWarming` | Opt-in cost-aware prompt cache refreshes during long tool runs; default `false`. Refreshes cost extra, stop when the task settles, and require a known model cache lifetime. |
